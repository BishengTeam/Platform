        ...
        <ScrollView className={styles.body} scrollY>
          ...
          {page?.items.map(item => (
            <View key={item.id} className={styles.card} onClick={() => Taro.navigateTo({ url: `/pages/quiz/mock?examId=${item.id}` })}>
              <View className={styles.header}><Text className={styles.status}>{STATUS_LABEL[item.status]}</Text><Text className={styles.score}>{item.score === null ? '-' : `${item.score.toFixed(1)} 分`}</Text></View>
              <Text className={styles.detail}>{item.question_count} 题 · 60 分钟 · {new Date(item.started_at).toLocaleString()}</Text>
              <Text className={styles.hint}>{item.status === 'in_progress' ? '点击继续作答' : item.status === 'abandoned' ? '放弃考试不展示答案和成绩' : '点击查看答题快照、答案和解析'}</Text>
            </View>
          ))}
